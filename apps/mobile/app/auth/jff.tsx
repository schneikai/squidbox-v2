...
export default function JFFAuthPage() {
  const { theme } = useTheme();
  const { ensureAuth } = useAuthVerification();
  const { onPlatformConnected } = usePlatformContext();
  const [isLoading, setIsLoading] = useState(false);

  const handleJFFLogin = useCallback(async () => {
    // Verify user is authenticated before starting OAuth flow
    if (!(await ensureAuth())) return;

    setIsLoading(true);
    try {
      // TODO: Implement JFF OAuth flow
      // For now, show a placeholder message
      Alert.alert(
        'Coming Soon',
        'JFF authentication is not yet implemented. This feature will be available in a future update.',
        [
          { text: 'OK', onPress: () => router.back() }
        ]
      );
    } catch (error) {
      console.error('Failed to start JFF authentication:', error);
      Alert.alert('Error', 'Failed to start JFF authentication. Please try again.');
    } finally {
      setIsLoading(false);
    }
  }, [ensureAuth]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={styles.content}>
        <Icon name="camera" type="feather" size={64} color="#FF6B6B" style={styles.icon} />
        ...
        <Button
          title="Connect to JFF"
          onPress={handleJFFLogin}
          loading={isLoading}
          disabled={isLoading}
